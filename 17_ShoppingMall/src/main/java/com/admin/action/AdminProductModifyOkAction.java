package com.admin.action;

import java.io.File;
import java.io.IOException;
import java.io.PrintWriter;

import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import com.oreilly.servlet.MultipartRequest;
import com.oreilly.servlet.multipart.DefaultFileRenamePolicy;
import com.shop.controller.Action;
import com.shop.controller.ActionForward;
import com.shop.model.ProductDAO;
import com.shop.model.ProductDTO;

public class AdminProductModifyOkAction implements Action {

	@Override
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws IOException {
		// 상품 등록 폼 페이지에서 넘어온 데이터들을
				// DB에 저장하는 비지니스 로직
				
				// 첨부파일이 저장될 위치(경로) 설정
				String saveFolder = "C:\\NCS\\git\\kh_JSP\\.metadata\\.plugins\\org.eclipse.wst.server.core\\tmp2\\wtpwebapps\\17_ShoppingMall\\image";
				int filesize = (1024 * 1024) * 50;	
				// 이미지 파일을 업로드를 위한 객체 생성
				MultipartRequest multi = new MultipartRequest(request, saveFolder, filesize, "UTF-8", new DefaultFileRenamePolicy());
				String []arr = multi.getParameterValues("all");
				//File file = multi.getFile("image");
				
				//번호, 코드, 상품명, 제조사, 수량, 가격, 사양, 소개, 포인트
				ProductDTO dto = new ProductDTO();
				
				dto.setNo(Integer.parseInt(arr[0]));
				dto.setCode(arr[1]);
				dto.setName(arr[2]);
				dto.setCompany(arr[3]);
				dto.setQty(Integer.parseInt(arr[4]));
				dto.setPrice(Integer.parseInt(arr[5]));
				dto.setSpec(arr[6]);
				dto.setCont(arr[7]);
				dto.setPoint(Integer.parseInt(arr[8]));
				
				String product_image = multi.getFilesystemName("p_image_new");
				if(product_image == null) {
					product_image = multi.getParameter("p_image_old");
				}
				System.out.println(product_image);
				File upload_file = multi.getFile(product_image); 
				//String fileName = upload_file.getName(); // 파일명 찾기
				//System.out.println(fileName);
				String reFile = arr[2] + "[" + product_image.substring(0, product_image.length()-4) + "].jpg";
				System.out.println(reFile);
				upload_file.renameTo(new File(saveFolder + "/" + reFile));
				dto.setImage(reFile);
				
				ProductDAO dao = ProductDAO.getInstnace();
				int check = dao.updateProduct(dto);
				
				ActionForward forward = new ActionForward();
				PrintWriter out = response.getWriter();
				if(check > 0) {
					forward.setRedirect(true);
					forward.setPath("admin_product_list.do");
				} else {
					out.println("<script>"
							+ "alert('수정 실패');"
							+ "history.back();"
							+ "</script>");
				}
				
				
				return forward;
	}

}
